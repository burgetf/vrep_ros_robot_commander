...
class Mission:
    ...
    def __init__(self, name):
        ...
        # timer to perform the gripping task
        self.timer = 0
        ...
        self.marker_pub = rospy.Publisher("marker_pub", Marker, queue_size=1)
        self.marker_pub2 = rospy.Publisher("marker_pub2", Marker, queue_size=1)
        self.marker_eff_path_pub = rospy.Publisher("marker_pub3", MarkerArray, queue_size=1)
        self.marker_obj_path_pub = rospy.Publisher("marker_pub4", MarkerArray, queue_size=1)
        ...
        # subscribe to "/visualization_marker" topic which is published by ar_track_alvar.
        rospy.Subscriber("/aruco_single/marker", Marker, self.marker_inf_cb, queue_size=1)
    ...
    def marker_inf_cb(self, data):
        ...
        # print "Position of object in cam frame : ", cam_object_T[0, 3], cam_object_T[1, 3], cam_object_T[2, 3]
        ...
        self.marker_pub2.publish(det_marker)

        # Transformation between base and object
        base_object_T = np.dot(np.dot(self.base_end_T, self.end_cam_T), cam_object_T)

        # final error computation based on the object pose in base frame and decided target pose for the object
        ...
        self.marker_pub.publish(det_marker)
        ...
        integration_time_step = 0.3
        pose_change = [speed * integration_time_step for speed in speed_cmd]
    ...
    def set_end_cam_T(self):
        ...
        delta_x = 0.03 #0.02
        delta_y = 0.13 #0.215 # it used to be 0.125
        delta_z = 0.04 #0.03
    ...
    def gripper_final_action(self):
        ...
        final_q_ik = self.kdl_kin.inverse(final_pose_T, np.array(self.end_eff_current_pose)+0.2)
    ...
    def error_computation(self, base_object, base_target):
		...
		base_object_euler = tr.euler_from_matrix(base_object_R, 'rxyz')

		base_target_R = tr.identity_matrix()
		base_target_R[0:3, 0:3] = base_target[0:3, 0:3]
		base_target_euler = tr.euler_from_matrix(base_target_R, 'rxyz')
		
		orientation_err = [a-b for a, b in zip(base_object_euler, base_target_euler)]
		"""
        ...
        base_object_R = tr.identity_matrix()
        base_object_R[0:3, 0:3] = base_object[0:3, 0:3]
        modification_matrix1 = np.array([[1,	        0,	        0,	      0],
                                         [0 ,            cos(np.pi/2),  -sin(np.pi/2), 0],
                                         [0,             sin(np.pi/2),   cos(np.pi/2), 0],
                                         [0,              0,             0,            1]])
        modification_matrix2 = np.array([[cos(np.pi),   -sin(np.pi),    0,	   0],
        ...
        goal_q2 = [real_base_object_quaternion[0], real_base_object_quaternion[1], real_base_object_quaternion[2], real_base_object_quaternion[3]]
